...
export default function AdminPenaltyItemsPage() {
  ...
  const [form, setForm] = useState({ name: "", penaltyMinutes: "", mode: "FIXED", unitLabel: "" });

  const [editId, setEditId] = useState<string | null>(null);
  const [editForm, setEditForm] = useState({ name: "", penaltyMinutes: "", mode: "FIXED", unitLabel: "" });
  ...
  async function handleCreate(e: React.FormEvent) {
    e.preventDefault();
    if (form.mode === "PROPORTIONAL" && !form.unitLabel.trim()) {
      showToast("比例モードの場合は単位ラベルを入力してください");
      return;
    }
    setSaving(true);
    const res = await fetch("/api/admin/penalty-items", {
      ...
      body: JSON.stringify({
        ...
        mode: form.mode,
        unitLabel: form.mode === "PROPORTIONAL" ? form.unitLabel : null,
      }),
    });
    setSaving(false);
    if (res.ok) {
      setForm({ name: "", penaltyMinutes: "", mode: "FIXED", unitLabel: "" });
      setShowForm(false);
      load();
      showToast("追加しました");
    }
  ...
  async function handleEditSave(id: string) {
    setSaving(true);
    await fetch(`/api/admin/penalty-items/${id}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        name: editForm.name,
        penaltyMinutes: Number(editForm.penaltyMinutes),
        mode: editForm.mode,
        unitLabel: editForm.mode === "PROPORTIONAL" ? editForm.unitLabel : null,
      }),
    });
    setSaving(false);
    setEditId(null);
  ...
  async function handleDelete(id: string, name: string) {
    ...
    await fetch(`/api/admin/penalty-items/${id}`, { method: "DELETE" });
    load();
    showToast("無効化しました");
  }
  ...
  async function handleSort(id: string, direction: "up" | "down") {
    ...
    await fetch(`/api/admin/penalty-items/${a.id}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ sortOrder: b.sortOrder }),
    });
    await fetch(`/api/admin/penalty-items/${b.id}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950">
      ...
      <main className="max-w-3xl mx-auto px-4 py-6 space-y-6">
        <div className="flex items-center justify-between">
          <h1 className="font-display text-2xl" style={{ color: "var(--expo-red)" }}>ペナルティ項目</h1>
          <button
        ...
        {showForm && (
          <form onSubmit={handleCreate} className="bg-white dark:bg-gray-900 rounded-2xl p-5 space-y-3 shadow" style={{ border: "2px solid #fca5a5" }}>
            <h2 className="font-black text-sm" style={{ color: "var(--expo-red)" }}>新規追加</h2>
            <div className="flex gap-3 flex-wrap">
              <input
                ...
                placeholder="項目名"
                required
                className="flex-1 rounded-xl px-3 py-2 text-sm font-bold bg-gray-50 dark:bg-gray-800 min-w-[200px]"
            ...
            {form.mode === "PROPORTIONAL" && (
              <input
                value={form.unitLabel}
                onChange={(e) => setForm({ ...form, unitLabel: e.target.value })}
                placeholder="単位ラベル（例: 分超過ごとに）"
                className="w-full rounded-xl px-3 py-2 text-sm font-bold bg-gray-50 dark:bg-gray-800"
                style={{ border: "2px solid #d1d5db" }}
              />
            )}
            <div className="flex gap-2 justify-end">
              ...
              <button type="submit" disabled={saving} className="px-4 py-2 rounded-xl font-black text-sm text-white" style={{ background: "var(--expo-red)" }}>
                {saving ? "保存中..." : "追加"}
              </button>
            </div>
          </form>
          ...
          {activeItems.map((item, idx) => (
            <div key={item.id} className="px-4 py-3 flex items-center gap-3">
              <div className="flex flex-col gap-0.5">
                ...
                <button onClick={() => handleSort(item.id, "down")} disabled={idx === activeItems.length - 1} className="text-gray-400 hover:text-gray-600 disabled:opacity-20">
              ...
              {editId === item.id ? (
                <div className="flex-1 flex items-center gap-2 flex-wrap">
                  ...
                  <input
                    ...
                    onChange={(e) => setEditForm({ ...editForm, penaltyMinutes: e.target.value })}
                  ...
                  <select
                    ...
                    className="rounded-xl px-2 py-1.5 text-sm font-bold bg-gray-50 dark:bg-gray-800"
                    style={{ border: "2px solid #fca5a5" }}
                  >
                    <option value="FIXED">固定</option>
                    <option value="PROPORTIONAL">比例</option>
                  </select>
                  {editForm.mode === "PROPORTIONAL" && (
                    <input
                      value={editForm.unitLabel}
                      onChange={(e) => setEditForm({ ...editForm, unitLabel: e.target.value })}
                      placeholder="単位ラベル"
                      className="rounded-xl px-2 py-1.5 text-sm font-bold bg-gray-50 dark:bg-gray-800 w-36"
                      style={{ border: "2px solid #fca5a5" }}
                    />
                  )}
                  <button onClick={() => handleEditSave(item.id)} disabled={saving} className="p-1.5 rounded-lg text-white" style={{ background: "var(--expo-red)" }}>
                ...
                <div
                  ...
                  onClick={() => { setEditId(item.id); setEditForm({ name: item.name, penaltyMinutes: String(item.penaltyMinutes), mode: item.mode, unitLabel: item.unitLabel ?? "" }); }}
                >
            ...
            <div className="bg-gray-100 dark:bg-gray-900 rounded-2xl overflow-hidden divide-y divide-gray-200 dark:divide-gray-800" style={{ border: "2px solid #e5e7eb" }}>
              {inactiveItems.map((item) => (
                <div key={item.id} className="px-4 py-3 flex items-center gap-3 opacity-60">
                  ...
                  <span className="text-xs text-gray-400">-{item.penaltyMinutes}分</span>
                  <button onClick={() => handleRestore(item.id)} className="text-xs font-black px-2 py-1 rounded-lg" style={{ color: "var(--expo-red)", border: "1px solid #fca5a5" }}>
                    有効化
                  </button>
                </div>
              ))}
